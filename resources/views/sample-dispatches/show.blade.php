            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="min-w-0">
                    ...
                    <p class="text-sm text-gray-600">
                        Sample <span class="font-semibold text-gray-900">{{ $dispatch->sample_id }}</span>
                        was dispatched on {{ $dispatch->dispatch_date->format('d M Y') }}
                        @if($dispatchTime)
                            at {{ $dispatchTime }}
                        @endif
                        by <span class="font-semibold text-gray-900">{{ $dispatch->dispatchedBy?->name ?? 'Unknown' }}</span>.
                    </p>
                </div>
                ...
                <div class="space-y-7 lg:col-span-7">
                    <section>
                        <div class="mb-4 flex items-center gap-2">
                            ...
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-gray-700">Sample and Route</h3>
                        </div>

                        <dl class="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Sample ID</dt>
                                <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $dispatch->sample_id }}</dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm font-medium text-gray-900">{{ $dispatch->origin_location }}</dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-700">{{ $dispatch->dispatch_date->format('d M Y') }}</dd>
                            </div>
                            <div>
                    ...
                    <section class="border-t border-gray-100 pt-6">
                        <div class="mb-4 flex items-center gap-2">
                            ...
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-gray-700">Driver and Dispatch Team</h3>
                        </div>
                        ...
                        <dl class="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-700">{{ $dispatch->dispatchedBy?->name ?? 'Unknown' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Registered Driver</dt>
                    ...
                    @if($dispatch->notes)
                        <section class="border-t border-gray-100 pt-6">
                            <div class="mb-2 flex items-center gap-2">
                                ...
                                <h3 class="text-sm font-semibold uppercase tracking-wide text-gray-700">Dispatch Notes</h3>
                            </div>
                ...
                <aside class="border-t border-gray-100 pt-7 lg:col-span-5 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
                    @if($dispatch->isReceived())
                        <section>
                            <div class="mb-4 flex items-center gap-2">
                                ...
                                <h3 class="text-sm font-semibold uppercase tracking-wide text-green-800">Receipt Confirmed</h3>
                            </div>

                            <dl class="grid grid-cols-1 gap-y-5 sm:grid-cols-2 sm:gap-x-8 lg:grid-cols-1">
                                <div>
                                    <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Received By</dt>
                                    <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $dispatch->receivedBy?->name ?? '-' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Received At</dt>
                                    <dd class="mt-1 text-sm text-gray-700">{{ $dispatch->received_at?->format('d M Y, H:i') ?? '-' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Condition on Arrival</dt>
                                    <dd class="mt-1 text-sm font-semibold {{ $conditionClass }}">{{ $dispatch->condition_on_arrival ?? '-' }}</dd>
                                </div>
                                @if($dispatch->rejection_reason)
                                    <div class="sm:col-span-2 lg:col-span-1">
                                        <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Rejection Reason</dt>
                                        <dd class="mt-1 text-sm leading-6 text-red-700">{{ $dispatch->rejection_reason }}</dd>
                                    </div>
                                @endif
                            </dl>
                        </section>
                    @elseif($canReceiveSamples)
                        <section x-data="{ condition: @js(old('condition_on_arrival')) }">
                            ...
                            <form method="POST" action="{{ route('sample-dispatches.receive', $dispatch) }}" class="space-y-4">
                                @csrf
                                ...
                                <div>
                                    ...
                                    <select name="condition_on_arrival" x-model="condition"
                                        ...
                                        @foreach($conditions as $cond)
                                            <option value="{{ $cond }}" {{ old('condition_on_arrival') === $cond ? 'selected' : '' }}>
                                                {{ $cond }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <textarea name="notes" rows="3"
                                              placeholder="Any observations at receipt"
                    ...
                    @else
                        <section>
                            <div class="mb-3 flex items-center gap-2">
                                ...
                                <h3 class="text-sm font-semibold uppercase tracking-wide text-amber-800">Awaiting Laboratory Receipt</h3>
                            </div>
                            <p class="text-sm leading-6 text-gray-600">
                                This sample has not been received yet. Laboratory users will see the receive action here.
                            </p>
                        </section>
                    @endif
                </aside>
                            ...
                            @foreach($history as $entry)
                                @php
                                    $eventStyles = match($entry->event) {
                                        ...
                                        default => ['bg-gray-100 text-gray-600', ucfirst(str_replace('_', ' ', $entry->event))],
                                    };

                                    $driverName = $entry->new_values['driver_name']
                                        ?? $entry->old_values['driver_name']
                                        ?? $dispatch->driver_name;

                                    $driverPhone = $entry->new_values['driver_phone']
                                        ?? $entry->old_values['driver_phone']
                                        ?? $dispatch->driver_phone;

                                    $details = '';
                                    if ($entry->event === 'created') {
                                        $details = ($entry->new_values['origin_location'] ?? $dispatch->origin_location)
                                            . ' to '
                                        ...
                                        if (!empty($entry->new_values['condition_on_arrival'])) {
                                            $details .= ' - Condition: ' . $entry->new_values['condition_on_arrival'];
                                        }
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3">
                                        <span class="inline-flex items-center rounded px-2 py-0.5 text-xs font-semibold {{ $eventStyles[0] }}">
                                            {{ $eventStyles[1] }}
                                        </span>
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-3 text-gray-700">
                                        {{ $entry->created_at->format('d M Y') }}
                                        <span class="block text-xs text-gray-400">{{ $entry->created_at->format('H:i:s') }}</span>
                                    </td>
