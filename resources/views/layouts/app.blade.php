    ...
    <div x-data="{ sidebarOpen: false }" class="flex min-h-screen">
        ...
        <aside
            ...
            <ul class="w-full space-y-1 flex-1">
                ...
                @php $isClinicalOnly = !auth()->user()?->isAdmin() && auth()->user()?->department === 'Clinical Operations'; @endphp
                ...
                <li>
                    <a href="{{ route('drivers.index') }}"
                        class="flex items-center py-2 px-3 rounded-lg hover:bg-gray-100
                              transition-colors duration-200 text-gray-700
                              {{ request()->routeIs('drivers.*') ? 'bg-orange-50 text-pbmc font-semibold' : '' }}">
                        <i data-feather="truck" class="w-5 h-5 mr-2 text-gray-500"></i>
                        Drivers
                    </a>
                </li>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="relative" id="notifWrap">
                            <button id="notifButton"
                                ...
                                @if ($unreadCount > 0)
                                    <span class="absolute top-1 right-1 min-w-[1rem] h-4 px-0.5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none">
                                    ...
                                    @forelse (auth()->user()?->notifications->take(5) ?? [] as $notif)
                                        ...
                                        <div class="px-4 py-3 {{ $isUnread ? 'bg-orange-50/40' : '' }}">
                                            <div class="flex items-center gap-2">
                                                <p class="text-xs font-semibold text-gray-800 flex-1">{{ $d['title'] ?? 'Notification' }}</p>
                                                @if ($isUnread)
                                                    <span class="w-1.5 h-1.5 rounded-full bg-orange-500"></span>
                                                @endif
                                            </div>
                                            <p class="text-xs text-gray-500 mt-0.5 truncate">{{ $d['message'] ?? '' }}</p>
